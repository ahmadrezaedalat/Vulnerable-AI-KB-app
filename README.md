...
## 15. Optional Tetragon monitoring

Tetragon is not required to run the application. It is an optional privileged
eBPF agent that runs once on every Kubernetes node and produces process,
network, and file-access security events.

The integration described here uses this data path:

```text
Tetragon agent DaemonSet
  -> tetragon.log JSON export
  -> export-stdout sidecar
  -> Kubernetes container log
  -> Splunk OpenTelemetry Collector agent DaemonSet
  -> HTTPS HEC
...
Create a small additional values file that marks Tetragon pods for the
dedicated Splunk log collector configured later:
...
### 15.4 Apply the application tracing policies

Apply the app-side network tracing policy:

```bash
kubectl apply -f k8s/40-tetragon-db-tracingpolicy.yaml
```

This policy watches unexpected non-PostgreSQL TCP connection attempts from the
application pod. Normal application-to-PostgreSQL traffic on destination port
`5432` is intentionally filtered out.

The database file and outbound-network policy is more verbose and is optional:

```bash
kubectl apply -f k8s/43-tetragon-db-file-tracingpolicy.yaml
```

It watches PostgreSQL data-directory activity and unexpected outbound TCP
connections from the database pod. PostgreSQL continuously accesses its data
directory, so this policy can generate a high event volume.

Restart the app after installing the policies so Tetragon observes its complete
...
If a `kubectl port-forward` was running, start it again after this rollout.
...
### 15.6 Splunk prerequisites

The following integration sends logs to Splunk Enterprise or Splunk Cloud
Platform. Before installing the collector, obtain:

- Splunk Enterprise 8.0 or newer, or a Splunk Cloud Platform stack.
- A Splunk index for the Tetragon events, such as `tetragon`.
- An enabled HTTP Event Collector (HEC) token authorized to write to that index.
- The complete HEC event endpoint. Examples:
  - Splunk Enterprise:
    `https://splunk.example.com:8088/services/collector`
  - Splunk Cloud: use the HEC URL provided for the specific stack; the hostname
    and port can differ from a self-managed Splunk instance.
- DNS, routing, firewall, and TLS trust from every Kubernetes node to the HEC
  endpoint.
- A server certificate whose hostname matches the HEC URL. Do not disable TLS
  verification for a real deployment.

In Splunk Web, create the index first, then create or select a HEC token and
allow that token to write to the index. Record the token once; do not add it to
a README, shell script, Helm values file, or source control.

Optionally test network reachability from the Ubuntu host before installing the
collector:
...
Replace the URL with the actual HEC host. A TLS or connection failure must be
fixed before the Kubernetes collector can send events.

### 15.7 Store the HEC token in Kubernetes

Create a namespace for the Splunk OpenTelemetry Collector:
...
Read the HEC token without echoing it, then create a Kubernetes Secret. Using a
shell variable prevents the literal token from being stored in shell history:

```bash
read -rsp 'Splunk HEC token: ' SPLUNK_HEC_TOKEN
echo

kubectl -n splunk-otel create secret generic splunk-otel-secrets \
...
Restrict access to this namespace and Secret through Kubernetes RBAC. A
Kubernetes Secret is base64-encoded, not encrypted by default; enable encryption
at rest for the Kubernetes API data store when required by the environment.

### 15.8 Configure the Splunk OpenTelemetry Collector

Create `splunk-otel-tetragon-values.yaml` with the following content. Replace
the cluster name, HEC endpoint, and index, but do not put the HEC token in this
file:

```yaml
clusterName: vulnerableapp-k8s

secret:
  create: false
  name: splunk-otel-secrets
...
`useSplunkIncludeAnnotation: true` makes this collector ingest only pod logs
marked with `splunk.com/include: "true"`. The Tetragon values file added that
annotation in Step 15.3. This prevents the dedicated integration from forwarding
every application and Kubernetes component log to the Tetragon index.

Tetragon's pod has multiple containers. The JSON security events come from the
`export-stdout` container, whose default Splunk sourcetype is normally
`kube:container:export-stdout`. The agent's operational messages remain
distinguishable as `kube:container:tetragon`.

If a Splunk OpenTelemetry Collector already exists in the cluster, merge these
settings into its managed Helm values and run `helm upgrade`; do not install a
second collector that would duplicate events.

### 15.9 Install the Splunk OpenTelemetry Collector

Add Splunk's chart repository:
...
Install the collector with the external Secret and values file:

```bash
helm upgrade --install splunk-otel-collector \
  splunk-otel-collector-chart/splunk-otel-collector \
  --namespace splunk-otel \
  --values splunk-otel-tetragon-values.yaml
```

Wait for the collector agent on every node:

```bash
kubectl rollout status -n splunk-otel \
  daemonset/splunk-otel-collector-agent --timeout=300s
kubectl get pods -n splunk-otel -o wide
```

Check its logs for HEC, authentication, TLS, queue, or file-log errors:

```bash
kubectl logs -n splunk-otel \
  daemonset/splunk-otel-collector-agent --tail=200
```

Do not use `splunkPlatform.insecureSkipVerify: true` to work around certificate
errors. Install the issuing CA and configure the chart's `splunkPlatform.caFile`
support instead.

### 15.10 Generate and find events in Splunk

Generate fresh activity after the collector is ready:
...
Start with this Splunk search:

```spl
index=tetragon sourcetype="kube:container:export-stdout"
```

Limit it to this lab's Kubernetes namespaces:

```spl
index=tetragon sourcetype="kube:container:export-stdout"
("vulnerableapp" OR "vulnerableapp-db")
| spath
```

Summarize common Tetragon event types:

```spl
index=tetragon sourcetype="kube:container:export-stdout"
| spath
| eval tetragon_event=case(
    isnotnull(process_exec), "process_exec",
    isnotnull(process_exit), "process_exit",
    isnotnull(process_kprobe), "process_kprobe",
    isnotnull(process_tracepoint), "process_tracepoint",
    true(), "other")
| stats count by tetragon_event
```
...
### 15.11 Splunk integration troubleshooting

If Tetragon events exist locally but not in Splunk, check the pipeline from left
to right:
...
# The include annotation survived the Helm deployment.
kubectl get pods -n kube-system \
  -l app.kubernetes.io/name=tetragon \
  -o jsonpath='{range .items[*]}{.metadata.name}{" => "}{.metadata.annotations.splunk\.com/include}{"\n"}{end}'

# The Splunk collector is ready and has no exporter errors.
...
- An incorrect HEC hostname, path, token, or index.
- The token is not authorized for the selected index.
- The HEC endpoint is unreachable from the node.
- A private certificate authority is not trusted by the collector.
- The `splunk.com/include` annotation is absent while include-only collection is
  enabled.
- Splunk search time range or index selection excludes the new events.
- Another collector is already ingesting the same container logs, causing
  duplicates in a different index.

### 15.12 Rotate the HEC token

Create a replacement token in Splunk, update the Kubernetes Secret, and restart
the collector agents:
...
unset SPLUNK_HEC_TOKEN
kubectl rollout restart -n splunk-otel \
...
### 15.13 Remove Tetragon and the Splunk collector
...
kubectl delete -f k8s/43-tetragon-db-file-tracingpolicy.yaml \
  --ignore-not-found
kubectl delete -f k8s/40-tetragon-db-tracingpolicy.yaml \
  --ignore-not-found
```

Then uninstall the collector and Tetragon:
...
- Splunk OpenTelemetry Collector Kubernetes chart:
  https://github.com/signalfx/splunk-otel-collector-chart
- Splunk collector advanced log configuration:
